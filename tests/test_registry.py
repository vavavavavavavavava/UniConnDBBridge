...
import pytest
from uniconndbbridge.registry import DriverRegistry, DialectInfo
...
class TestDriverRegistry:
    ...
    def test_validate_driver(self):
        ...
        assert not DriverRegistry.validate_driver("postgresql", "asyncpg", is_async=False)
        assert not DriverRegistry.validate_driver("postgresql", "psycopg2", is_async=True)
        assert not DriverRegistry.validate_driver("postgresql", "unknown_driver", is_async=False)
        assert not DriverRegistry.validate_driver("unknown_dialect", "psycopg2", is_async=False)
    ...
    def test_check_driver_requirements(self):
        ...
        requirements_met, missing = DriverRegistry.check_driver_requirements("sqlite", "pysqlite")
        assert requirements_met is True
        assert len(missing) == 0
        
        # Test unknown dialect
        requirements_met, missing = DriverRegistry.check_driver_requirements("unknown", "unknown")
