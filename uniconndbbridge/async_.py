...
from typing import Union, Optional, Dict, Any, AsyncGenerator, List, Sequence
...
class AsyncDatabaseManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized AsyncDatabaseManager with URL: {self._mask_url(self._url)}")
    ...
    def _ensure_async_url(self, url: str) -> str:
        ...
        if "+" in scheme:
            ...
            if dialect_info and driver in dialect_info.async_drivers:
                ...
                try:
                    best_async_driver = DriverRegistry.get_best_driver(dialect, is_async=True)
    ...
    def engine(self) -> AsyncEngine:
        ...
        if self._engine is None:
            raise RuntimeError("Engine not created. Call create_engine() first or use async context manager.")
        return self._engine
    ...
    async def execute(self, statement: Union[str, Executable], parameters: Optional[Dict[str, Any]] = None):
    ...
    async def fetch_one(self, statement: Union[str, Executable], parameters: Optional[Dict[str, Any]] = None):
        """Execute a statement and fetch one result asynchronously."""
        result = await self.execute(statement, parameters)
        return result.fetchone()
    
    async def fetch_many(self, statement: Union[str, Executable], 
        ...
        try:
            async with self.connection() as conn:
                ...
                if self._engine.dialect.name == "postgresql":
                    ...
                    result = await conn.execute(query)
                    return [row[1] for row in result.fetchall()]  # Column name is index 1
                else:
                    # Fallback to reflection
                    metadata = MetaData()
                    await conn.run_sync(metadata.reflect, schema=schema, only=[table_name])
    ...
    @classmethod
    async def discover_connection_async(cls, *,
                                       user: str,
                                       password: str,
                                       host: str = "localhost",
