...
from .exceptions import DiscoveryError, wrap_sqlalchemy_error
...
class ConnectionDiscovery:
    ...
    def discover_connection(cls, *,
        ...
        logger.error(f"Failed to discover any working connection. Tried: {list(dialects)}")
    ...
    def _try_dialect(cls, *,
        ...
        available_drivers = DriverRegistry.get_available_drivers(dialect, is_async=False)
    ...
    def _build_url(cls, *,
        ...
        url_parts.extend([
            "://",
            f"{encoded_user}:{encoded_password}@{host}"
        ])
    ...
    def check_dialect_availability(cls, dialect: str) -> Dict[str, bool]:
        ...
        for driver in dialect_info.async_drivers:
            availability[f"{driver} (async)"] = DriverRegistry.is_driver_available(driver)
    ...
    async def discover_connection_async(cls, *,
        ...
        # No working connection found
        logger.error(f"Failed to discover any working async connection. Tried: {list(dialects)}")
        raise DiscoveryError(
    ...
    @classmethod
    async def _try_dialect_async(cls, *,
                                dialect: str,
                                user: str,
                                password: str,
                                host: str,
