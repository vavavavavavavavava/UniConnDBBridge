...
def wrap_sqlalchemy_error(
    original_error: Exception, operation: Optional[str] = None
) -> UniConnDBError:
    ...
    if isinstance(original_error, DisconnectionError):
        return ConnectionError(
            f"Database connection lost: {error_message}", original_error=original_error
        )

    elif isinstance(original_error, OperationalError):
        if "authentication" in error_message.lower():
            return AuthenticationError(
                f"Authentication failed: {error_message}", original_error=original_error
            )
        else:
            return ConnectionError(
                f"Operational error: {error_message}", original_error=original_error
            )

    elif isinstance(original_error, SQLTimeoutError):
        return PoolError(
            f"Connection timeout: {error_message}", original_error=original_error
        )
        ...
        if "transaction" in error_message.lower():
            ...
            return ConfigurationError(
                f"Invalid request: {error_message}", original_error=original_error
            )
